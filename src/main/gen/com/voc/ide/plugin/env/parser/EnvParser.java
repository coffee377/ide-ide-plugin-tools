...
package com.voc.ide.plugin.env.parser;
...
public class EnvParser implements PsiParser, LightPsiParser {
  ...
  static boolean parse_root_(IElementType t, PsiBuilder b, int l) {
    return envFile(b, l + 1);
  }

  /* ********************************************************** */
  // item_*
  static boolean envFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "envFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!item_(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "envFile", c)) break;
    }
    return true;
  }
